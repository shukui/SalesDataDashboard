...
st.set_page_config(page_title="SalesDashboard", page_icon=":bar_chart:",layout="wide")

st.title(" :books: Sales Data Dashboard")
st.markdown('<style>div.block-container{padding-top:1rem;}</style>',unsafe_allow_html=True)

fl = st.file_uploader(":file_folder: Upload",type=(["csv","txt","xlsx","xls"]))
if fl is not None:
    filename = fl.name
    st.write(filename)
    df = pd.read_csv(filename, encoding = "ISO-8859-1")
else:
    os.chdir(r"C:\Users\zhang\source\repos\SalesDataDashboard\SalesDataDashboard")
    df = pd.read_csv("SalesDataSample.csv", encoding = "ISO-8859-1")
...
# Filter the data based on Region, country and City
...
with st.expander("Summary_Table"):
    df_sample = df[0:5][["Region","Country","City","Category","Sales","Profit","Quantity"]]
